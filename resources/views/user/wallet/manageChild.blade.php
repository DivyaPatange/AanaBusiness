...
@if($loop->depth <= 7)
    <?php
        ...
        if(!empty($plan1))
        {
            $income = DB::table('incomes')->where('child_id', $child->id)->where('user_id', Auth::user()->id)->first();
            if(empty($income))
            {
            ...
            $joinerLevel = DB::table('joiner_levels')->where('user_id', Auth::user()->id)->where('level', $loop->depth)->first();
            if(empty($joinerLevel))
            {
                ...
                $result = DB::table('joiner_levels')->where('user_id', Auth::user()->id)->where('level', $loop->depth)->update(['joiner_added' => $items, 'status' => $status]);
            }
        }
    ?>
    @if(!empty($plan1))
