...
<div class="row">
    ...
    <div class="col-md-6 col-lg-6 col-xl-3">
        <div class="media widget-media p-4 bg-white border">
            ...
            <div class="media-body align-self-center">
                <h5 class="text-primary mb-2">Income Settlement</h5>
                <p>@if($userPlan->income_settlement == 1) Per Month @else After 6 Month @endif</p>
            </div>
        </div>
    </div>
    <div class="col-md-6 col-lg-6 col-xl-3">
...
<div class="row">
    ...
    <?php 
        $paymentSettlement = DB::table('payment_settlements')->where('user_id', $userProfile->id)->get();
    ?>
    <div class="col-xl-8 col-md-8 col-12">
        <div class="card widget-block p-4 rounded bg-white border">
            <div class="card-block">
                <h4 class="text-primary my-2">&#8377;{{ $paymentSettlement->sum('total') }}</h4>
                <p class="pb-3">Total Income</p>
                <div class="progress my-2" style="height: 5px;">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 80%;" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Settled Status</th>
                            <th scope="col">Settled Date</th>
                            <th scope="col">Amount</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($paymentSettlement as $key => $payment)
                        <tr>
                            <td>{{ ++$key }}</td>
                            <td>{{ date('F', mktime(0,0,0,$payment->month, 1, date('Y'))) }}/{{ $payment->year }}</td>
                            <td>&#8377;{{ $payment->total }}</td>
                            <td>@if($payment->settled_status == 1) Settled @else  Not Settled @endif</td>
                            <td>{{ date('Y-m-d', strtotime($payment->settled_date)) }}</td>
                        </tr>
                        @endforeach
                        <tr>
                            <td colspan="4">Total</td>
                            <td >&#8377;{{ $paymentSettlement->sum('total') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
